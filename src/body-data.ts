...
export function imageTransform(f:Fighter){
 ...
 return {width:data.width,height:data.height,originX:data.origin[0],originY:data.origin[1],scaleX:data.displayHeight/data.height,scaleY:(f.character.key!=='science'&&f.crouching?85:data.displayHeight)/data.height,offsetY:data.offsetY,angle:f.hp===0?(f.character.key==='science'?0:f.dir*75):f.stun&&!f.blocked?-f.dir*9:0};
}
